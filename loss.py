...
# 自己的SSIM，
class MySSIM(torch.nn.Module):
    def __init__(self, window_size=11, size_average=True):
        super(MySSIM, self).__init__()
        self.window_size = window_size
        self.size_average = size_average
        self.channel = 1
        self.window = create_window(window_size, self.channel)

    def forward(self, img1, img2, mask=1, model='L'):
        (_, channel, _, _) = img1.size()
        if channel == self.channel and self.window.data.type() == img1.data.type():
            window = self.window
        else:
        ...
        if model=='L':
            return L_ssim(img1, img2, window, self.window_size, channel, self.size_average, mask=mask)
        else:
            return D_ssim(img1, img2, window, self.window_size, channel, self.size_average, mask=mask)

def L_ssim(img1, img2, window, window_size, channel, size_average=True, mask=1):
    mu1 = F.conv2d(img1, window, padding=window_size//2, groups=channel)
    mu2 = F.conv2d(img2, window, padding=window_size//2, groups=channel)
...
def D_ssim(img1, img2, window, window_size, channel, size_average=True, mask=1):
    mu1 = F.conv2d(img1, window, padding=window_size//2, groups=channel)
    mu2 = F.conv2d(img2, window, padding=window_size//2, groups=channel)
...
# def intLoss(img,window_size=7):
#     kernel = np.ndarray(16*window_size*window_size).reshape(16,1,window_size,window_size)
#     kernel = np.ones_like(kernel)
#     grad_img = torch.nn.Conv2d(1,1,7,stride=7,padding=0,dilation=1, groups=1,bias=False)
#     grad_img.weight.data = kernel
#     grad_img = grad_img(img)
#     return grad_img
def intLoss(img):
